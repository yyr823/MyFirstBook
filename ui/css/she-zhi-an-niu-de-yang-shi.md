...
zoom:1;/*normal|number 设置或检索对象的缩放比例(是IE浏览器的专有属性，Firefox等浏览器不支撑)  */

outline-width:medium;/*整个轮廓的宽度*/

outline-color:inherit;/*轮廓的颜色*/

font-size-adjust:none;/*number|none|inherit 控制字体大小  (只有Firefox支持)*/

font-stretch:normal;/*允许你使文字变宽或变窄 (目前没有主流浏览器支持font-Stretch属性)*/
